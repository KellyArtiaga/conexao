...
export class NavContentComponent implements OnInit, AfterViewInit {
  ...
  constructor(public nav: NavigationItem, private zone: NgZone, private location: Location, private service: OperadoresService, private auth: AuthGuardService) {
    this.dattaConfig = DattaConfig.config;
    this.windowWidth = window.innerWidth;

    this.navigation = this.filterRoutesByUserRole(this.whithoutReactive(this.nav.get()));
  ...
  whithoutReactive(json:any) {
    return JSON.parse(JSON.stringify(json));
  }

  filterRoutesByUserRole(nav: any) {
    var newNav = [];

    nav.map((parent, index) => {
      var validChildren = []

      if (parent.children) {
        parent.children.map(child => {
          if (child.url && this.auth.canActivate(child.url)) {
    ...
    console.log("nav:", newNav, atob(localStorage.getItem('tipoUsuario')))
    return newNav;
  }
