...
export class AppService {
    ...
    getServerAPIEncode(): string {
        return `${AppConfig.API_ENCODE}/${this.resource}`;
    }
    ...
    /* getHttpHeaderOptionFormDataToken(): any {
        const httpOptions = {
            headers: new HttpHeaders({
                'client_id': `${AppConfig.CLIENT_ID}`,
                'access_token': `${this.userContext.getToken()}`
    ...
    getHttpHeaderOptionMultipart(): any {
        const httpOptions = {
            headers: new HttpHeaders({
                'Authorization': `Bearer ${this.userContext.getTokenBearer()}`
            })
        };
        return httpOptions;
    }
