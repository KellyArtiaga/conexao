...
export class EmpresasStatusComponent implements OnInit {
  form: FormGroup;
  public oldData: any = {};
  status= '';
  data = '';
    ...
    if (form.valid) {
      ...
      this.service.post(body)
          .subscribe(res => {
            ...
            this.router.navigateByUrl('cadastro/empresas/editar/' + this.idEmpresa);
          }, err => {
    ...
    if (form.valid) {
      if(this.status === this.form.value.ativo){
        ...
        this.service.postStatus(body)
            .subscribe(res => {
              this.snackBar.success('Alteração cadastrada com sucesso!', 3500, 'X');
              this.router.navigate(['cadastro/empresas/editar/' + this.idEmpresa]);
            }, err => {
              this.snackBar.error(err, 3500, 'X');
            });
        }
