...
@Component({
  selector: 'app-operador-contrato',
  templateUrl: './operador-contrato.component.html',
  styleUrls: ['./operador-contrato.component.scss'],
  providers: [DatePipe,{provide: LOCALE_ID, useValue: 'pt-BR' }]
})

export class OperadorContratoComponent implements OnInit {
  ...
  isEditing() {
	...
	// getDadosContrato() {
	// 	if (this.operadorId) {
	// 	this.service.getDadosContrato(this.operadorId)
	// 	.subscribe(res => {
  ...
  async save(form?:any) {
		const body = {
			...
			idContratante:  parseInt(this.operadorId),
			urlContrato: '',
			nome:  this.form.get('nome').value,
			email: this.form.get('email').value ? this.form.get('email').value.toLowerCase() : "",
			cpf:  this.form.get('cpf').value ? this.form.get('cpf').value.split('.').join('').split('-').join('') : '',
			celular:  this.form.get('celular').value ? this.form.get('celular').value.split('(').join('').split('-').join('').split(')').join('').split(' ').join('') : '' ,
		}
      ...
      if (this.form.value.files && this.form.value.files.length > 0) {
        ...
        body.urlContrato = imgInfo.data;
			...
			.subscribe(res => {
				...
				this.router.navigateByUrl('cadastro/operadores/editar/'+res.data.idContratante);
			}, err => {
  ...
  async saveContractPdf() {
	...
	printContract() {
    const printContents = document.getElementById('pdf-src').innerHTML;
  }
}
